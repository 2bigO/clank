...
- `qwopus-pi`: local OpenAI-compatible `llama-server` alias `qwopus-pi`,
  currently `--ctx-size 262144` with llama.cpp fit-controlled partial ROCm
  offload.
...
- Full forced Qwopus offload can stall after tensor loading; keep
  `--gpu-layers -1` unless retesting the offload plan.
