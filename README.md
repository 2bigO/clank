...
Qwopus runs at native `--ctx-size 262144` with `--gpu-layers -1` so
llama.cpp `fit` can choose a ROCm-safe partial offload. Forcing
`--gpu-layers 999` can wedge after tensor offload.
